...
1. Deletes anchors used for design puposes e.g making glyphs built with
   components
...
def _rename_anchors(glyph):
    ...
    for before_anchor, after_anchor in anchors:
        rename_anchor(glyph, before_anchor, after_anchor)


def _script_already_used(glyph):
    """Check if a user has already executed the script on the .ufo.
    Executing the script multiple times on the same .ufo will lead to
    unforeseen modifications."""
    prev_added_anc = glyph.xpath("//point[@name='parent_top']")
    if prev_added_anc:
        return True
    return False
...
def main(ufo_path):
    if not ufo_path.endswith('ufo'):
        raise Exception('{} is not a ufo'.format(ufo_path))
    glifs = glob(os.path.join(ufo_path, 'glyphs', '*.glif'))

    # check script has not already been run on this font
    a_glyph = etree.parse(os.path.join(ufo_path, 'glyphs', 'a.glif'))
    if _script_already_used(a_glyph):
        raise Exception('Script has already been used')

    for path in glifs:
        glyph = etree.parse(path)
        glyph_name = glyph.getroot().attrib['name']

        _delete_design_anchors(glyph)
        _rename_anchors(glyph)

        if glyph_name in ['cedilla', 'commaaccent', 'ogonek']:
            delete_anchor(glyph, 'bottom')

        if glyph_name in ['uni1ABB', 'uni1ABC']:
            delete_anchor(glyph, '_top')

        if glyph_name in DUPLICATE_ANCHOR_GLYPHS:
            duplicate_anchor(glyph, "top", "parent_top")
        glyph.write(path)

    print('Fixed anchors: {}'.format(ufo_path))


if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise Exception("Include path to ufo")
    else:
        ufo_path = sys.argv[1]
        main(sys.argv[1])